...
import { View, Text, ScrollView, TouchableOpacity, Image, Share, Linking, TextInput, KeyboardAvoidingView, Platform, Alert, Dimensions, ActivityIndicator } from 'react-native';
import { useLocalSearchParams, useRouter } from 'expo-router';
import { SafeAreaView } from 'react-native-safe-area-context';
import { ArrowLeft, Heart, Share2, Star, MapPin, ShieldCheck, Info, Wifi, Car, Wind, Tv, Sofa, WashingMachine, Utensils, Refrigerator, Phone, Calendar, Send } from 'lucide-react-native';
...
export default function RoomDetailScreen() {
    ...
    const fetchDetail = async () => {
      try {
        ...
        const mappedProperty = {
          ...
          area: prop.area || 25,
          image: prop.image || 'https://images.unsplash.com/photo-1522708323590-d24dbb6b0267?w=800',
          images: prop.images && prop.images.length > 0 ? prop.images : [prop.image || 'https://images.unsplash.com/photo-1522708323590-d24dbb6b0267?w=800'],
          available: prop.available !== undefined ? prop.available : true,
          verificationLevel: prop.verificationLevel || (prop.badgeAwarded === 'verified' ? 'verified' : 'none'),
          verifiedAt: prop.verifiedAt || prop.createdAt,
          locationAccuracy: prop.locationAccuracy || 5,
          location: prop.location && Array.isArray(prop.location) ? [prop.location[0], prop.location[1]] : [106.7009, 10.7769],
          amenities: {
          ...
          landlordId: {
            ...
            avatar: prop.landlordId?.avatar || 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=100',
        ...
        const mappedReviews = (reviewsRes.data || []).map((r: any) => ({
          id: r._id || r.id,
          userName: r.userId?.fullName || r.userId?.username || 'Người dùng MapHome',
          userAvatar: r.userId?.avatar || 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=100',
          rating: r.rating || 5,
          content: r.comment || '',
          ...
          onPress: async () => {
            try {
              ...
              if (res.status === 201 || res.status === 200) {
                Alert.alert('Thành công', 'Đã đặt lịch hẹn xem phòng trên hệ thống. Chủ trọ sẽ sớm liên hệ.');
              }
            } catch (err: any) {
              Alert.alert('Lỗi', err.response?.data?.message || 'Không thể đặt lịch. Bạn cần đăng nhập trước.');
            }
          }
        }
      ]
  ...
  const handleSendReview = async () => {
    ...
    try {
      ...
      Alert.alert('Lỗi', e.response?.data?.message || 'Không thể gửi đánh giá. Bạn đã đăng nhập chưa?');
    }
  };
  ...
  if (loading) {
    return (
      <SafeAreaView className="flex-1 bg-white items-center justify-center p-6">
        ...
        <Text className="text-slate-500 mt-2 font-semibold">Đang tải thông tin phòng trọ...</Text>
      </SafeAreaView>
    );
  }
  ...
  if (!property) {
    return (
      <SafeAreaView className="flex-1 bg-white items-center justify-center p-6">
        <Text className="text-xl font-bold text-emerald-950 mb-4">Không tìm thấy phòng trọ</Text>
        <TouchableOpacity 
          onPress={() => router.back()}
  ...
  return (
    ...
    >
      <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
        ...
        <View className="relative h-72 w-full bg-slate-900">
          ...
          <SafeAreaView className="absolute top-0 left-0 right-0 p-4 flex-row justify-between items-center z-10">
            ...
            <View className="flex-row space-x-2">
              ...
              <TouchableOpacity 
                ...
                className="w-10 h-10 bg-black/40 rounded-full items-center justify-center"
              >
                <Heart size={20} color={isFavorite ? '#f43f5e' : 'white'} fill={isFavorite ? '#f43f5e' : 'transparent'} />
              </TouchableOpacity>
            </View>
          </SafeAreaView>
          ...
          <View className="absolute bottom-4 right-4 bg-black/60 px-3 py-1 rounded-full">
            <Text className="text-white text-xs font-bold">1 / {property.images.length}</Text>
          </View>
        </View>
        ...
        <View className="px-4 py-6">
          ...
          <View className="flex-row items-center mb-4">
            ...
            <Text className="text-slate-600 text-sm ml-1 flex-1" numberOfLines={2}>
              {property.address}
            </Text>
          </View>
          ...
          <View className="flex-row flex-wrap gap-2 mb-6">
            {property.verificationLevel === 'verified' ? (
              <View className="flex-row items-center bg-emerald-50 px-3 py-1.5 rounded-xl border border-emerald-200">
                ...
                <Text className="text-xs text-emerald-700 font-bold ml-1">Đã xác thực GPS (±{property.locationAccuracy}m)</Text>
              </View>
            ) : (
              <View className="flex-row items-center bg-amber-50 px-3 py-1.5 rounded-xl border border-amber-200">
                <Info size={14} color="#d97706" />
                <Text className="text-xs text-amber-700 font-bold ml-1">Chưa xác thực</Text>
              </View>
            )}
            <View className="flex-row items-center bg-amber-50 px-3 py-1.5 rounded-xl border border-amber-200">
              <Star size={14} color="#d97706" fill="#d97706" />
              <Text className="text-xs text-amber-700 font-bold ml-1">{avgRating} ({reviewsList.length} đánh giá)</Text>
            </View>
          </View>
          ...
          <View className="flex-row bg-white rounded-3xl p-5 border border-slate-100 shadow-sm justify-around mb-6">
            <View className="items-center">
              <Text className="text-xs text-slate-400 font-medium mb-1">Giá thuê</Text>
              <Text className="text-xl font-black text-emerald-600">{property.price.toLocaleString('vi-VN')}đ</Text>
              <Text className="text-[10px] text-slate-500 font-bold uppercase mt-0.5">/ tháng</Text>
            </View>
            <View className="w-px bg-slate-100" />
            <View className="items-center">
              <Text className="text-xs text-slate-400 font-medium mb-1">Diện tích</Text>
              <Text className="text-xl font-black text-emerald-950">{property.area} m²</Text>
              <Text className="text-[10px] text-slate-500 font-bold uppercase mt-0.5">Diện tích phòng</Text>
            </View>
          </View>
          ...
          <View className="flex-row border-b border-slate-200 mb-6 bg-white rounded-2xl p-1 shadow-sm">
            ...
            >
              <Text className={`font-bold text-sm ${activeTab === 'info' ? 'text-white' : 'text-slate-500'}`}>Thông tin phòng</Text>
            </TouchableOpacity>
            <TouchableOpacity 
              onPress={() => setActiveTab('reviews')}
              className={`flex-1 py-3 rounded-xl items-center justify-center flex-row ${activeTab === 'reviews' ? 'bg-emerald-600' : ''}`}
            >
              <Text className={`font-bold text-sm ${activeTab === 'reviews' ? 'text-white' : 'text-slate-500'}`}>Đánh giá ({reviewsList.length})</Text>
            </TouchableOpacity>
          </View>
          ...
          {activeTab === 'info' ? (
            <View className="space-y-6">
              ...
              <View className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100">
                <Text className="text-lg font-black text-emerald-950 mb-4">Tiện nghi phòng trọ</Text>
                
                <View className="flex-row flex-wrap">
                  {activeAmenities.map((amenity) => (
                    <View key={amenity} className="w-1/2 flex-row items-center mb-4 pr-2">
                      <View className="w-8 h-8 rounded-xl bg-emerald-50 items-center justify-center text-emerald-600 mr-2 border border-emerald-100">
                        {amenity === 'wifi' && <Wifi size={16} color="#059669" />}
                        {amenity === 'parking' && <Car size={16} color="#059669" />}
                        {amenity === 'ac' && <Wind size={16} color="#059669" />}
                        {amenity === 'water' && <Info size={16} color="#059669" />}
                        {amenity === 'tv' && <Tv size={16} color="#059669" />}
                        {amenity === 'furniture' && <Sofa size={16} color="#059669" />}
                        {amenity === 'washingMachine' && <WashingMachine size={16} color="#059669" />}
                        {amenity === 'kitchen' && <Utensils size={16} color="#059669" />}
                        {amenity === 'refrigerator' && <Refrigerator size={16} color="#059669" />}
                      </View>
              ...
              {/* Description */}
              <View className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100">
                <Text className="text-lg font-black text-emerald-950 mb-3">Mô tả chi tiết</Text>
              ...
              {property.verificationLevel === 'verified' && (
                <View className="bg-emerald-50 rounded-3xl p-5 border border-emerald-100">
                  <Text className="text-emerald-900 font-black text-base mb-1">✓ Vị trí đã xác thực tại chỗ</Text>
                  <Text className="text-emerald-700 text-xs font-medium leading-relaxed">
                    Chủ trọ đã ghim tọa độ này trực tiếp tại địa chỉ phòng trọ qua thiết bị di động. Độ chính xác định vị ghi nhận là ±{property.locationAccuracy} mét. Bạn có thể tin cậy 100% vào bản đồ chỉ đường.
                  </Text>
                </View>
              )}
              ...
              <View className="bg-white rounded-3xl p-4 shadow-sm border border-slate-100">
                <Text className="text-lg font-black text-emerald-950 mb-3">Bản đồ vị trí</Text>
              ...
              <View className="bg-white rounded-3xl p-5 shadow-sm border border-slate-100 flex-row items-center justify-between">
                <View className="flex-row items-center flex-1 mr-4">
                  ...
                  <View className="flex-1">
                    <Text className="text-xs text-slate-400 font-bold uppercase tracking-wider">Chủ trọ đăng tin</Text>
                    <Text className="text-base font-black text-emerald-950">{property.landlordId.fullName}</Text>
                    <Text className="text-xs text-slate-500 font-medium">Hoạt động tích cực</Text>
                  </View>
                </View>
                <TouchableOpacity 
                  onPress={handleCall}
            ...
            <View className="space-y-6">
              {/* Review submit box */}
              <View className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100">
                <Text className="text-lg font-black text-emerald-950 mb-3">Gửi đánh giá của bạn</Text>
                
                {/* Rating selection */}
                <View className="flex-row items-center mb-4">
                  <Text className="text-slate-500 font-bold mr-3 text-sm">Điểm đánh giá:</Text>
              ...
              <View className="space-y-4">
                {reviewsList.length === 0 ? (
                  <View className="bg-white rounded-3xl p-8 shadow-sm border border-slate-100 items-center justify-center">
                    <Text className="text-slate-400 font-bold text-center">Chưa có đánh giá nào cho phòng trọ này.</Text>
                  </View>
                ) : (
                  reviewsList.map((rev) => (
                    <View key={rev.id} className="bg-white rounded-3xl p-5 shadow-sm border border-slate-100">
                      <View className="flex-row justify-between items-start mb-3">
                        <View className="flex-row items-center">
                          ...
                          />
                          <View>
                            <Text className="text-sm font-black text-emerald-950">{rev.userName}</Text>
                        ...
                        <View className="flex-row items-center bg-amber-50 border border-amber-100 px-2 py-0.5 rounded-lg">
                          ...
                          <Text className="text-xs font-black text-amber-700 ml-1">{rev.rating}</Text>
                        </View>
                      </View>
      ...
      <SafeAreaView edges={['bottom']} className="absolute bottom-0 left-0 right-0 bg-white border-t border-slate-100 px-4 py-3 flex-row shadow-[0_-8px_30px_rgb(0,0,0,0.04)]">
        <TouchableOpacity 
          onPress={handleCall}
        ...
        >
          ...
          <Text className="text-slate-700 font-black text-base ml-2">Gọi điện</Text>
        </TouchableOpacity>
        <TouchableOpacity 
          onPress={handleBook}
        ...
        >
          ...
          <Text className="text-white font-black text-base ml-2">Đặt lịch hẹn</Text>
        </TouchableOpacity>
      </SafeAreaView>
    </KeyboardAvoidingView>
  );
