...
interface AuthContextType {
  user: User | null;
  loading: boolean;
  login: (usernameOrEmail: string, password: string) => Promise<{ success: boolean; message?: string; role?: string }>;
  register: (data: any) => Promise<{ success: boolean; message?: string }>;
  logout: () => Promise<void>;
  updateUser: (userData: User) => Promise<void>;
  toggleFavorite: (propertyId: string) => Promise<{ success: boolean; action?: 'added' | 'removed'; message?: string }>;
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const initializeAuth = async () => {
      try {
        ...
        // Clear if token is invalid or request failed with auth error
  ...
  const toggleFavorite = async (propertyId: string) => {
    ...
    try {
      ...
      return { 
        success: false, 
        message: err.response?.data?.message || 'Không thể cập nhật danh sách yêu thích.' 
      };
    }
  };
